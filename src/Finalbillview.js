...
const Finalbillview = (props) => {
	const [ab, setab] = useState({ ab: props.location.state.meter });
	const { meter, id, amount, status, reading, billingDate, eCharge, fixCharge, lReading, lastPayDate, pandding, usage, unitPrize } = props.location.state;
	return (
		<>
			...
			<div className="invoice-box">
				...
				{(status === "panding") ?
					<Link to={{ pathname: "/upay", state: { ab, id } }}><input className="btn btn-dark" value="pay" readOnly /></Link> : (console.log())}<p></p>
				<Link to={{ pathname: "/udata", state: ab }}><input className="btn btn-dark" value="back" readOnly /></Link>
			</div>
		</>
	)
}
export default Finalbillview;